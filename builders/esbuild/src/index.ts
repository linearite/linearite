    ...
    ctx.on('build:item', async (workspace, opts) => {
      ...
      await Promise.all(format.map(async (format) => {
        ...
        if (format === 'esm' && !workspace.meta.module) {
          console.warn('not found module field in package.json, will fallback to `${outdir}/index.mjs`')
        }
        if (format === 'cjs' && !workspace.meta.main) {
          console.warn('not found main field in package.json, will fallback to `${outdir}/index.js`')
        }
        await build({
          entryPoints: [dir('src/index.ts')],
          outfile: {
            esm: dir(workspace.meta.module ?? 'dist/index.mjs'),
            cjs: dir(workspace.meta.main   ?? 'dist/index.cjs'),
